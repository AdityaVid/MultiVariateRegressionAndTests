...
function get_features(pm::Polynomial2Model, _X::AbstractMatrix)
    ...
    # Degree 2 terms (including cross-terms)
    for i in 1:d
        Φ[:, idx] .= X[:, i] .^ 2 # Square terms
        idx += 1
        for j in i+1:d
            Φ[:, idx] .= X[:, i] .* X[:, j] # Cross-terms
            idx += 1
        end
    end
...
function loss(lm::AbstractModel, mse::MSE, X, Y)
    ...
    mse_loss = sum((predictions .- Y).^2) / (2 * n)  # Calculate MSE
    return mse_loss
end
...
function update!(lm::LinearModel, 
	...
	#### BEGIN SOLUTION
	opt.gbar += g.^2  # accumulate squared gradients
    lm.w .-= (opt.η ./ sqrt.(opt.gbar .+ opt.ϵ)) .* g  # update weights
...
function epoch!(mbgd::MiniBatchSGD, model::LinearModel, lossfunc, opt, X, Y)
    ...
    # Determine the number of minibatches
    num_batches = ceil(Int, size(X, 1) / mbgd.b)

    # Iterate over minibatches
    for i in 1:num_batches
        # Determine indices for the minibatch
        batch_start = (i - 1) * mbgd.b + 1
        batch_end = min(i * mbgd.b, size(X, 1))
        ...
        # Update the model using the provided update! function
        update!(model, lossfunc, opt, X_batch, Y_batch)
    end
end
